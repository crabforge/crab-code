...
> 版本：v2.1
> 更新：2026-04-06
> 状态：Phase 1 完成, Phase 2 进行中（清理过度工程模块后）
...
Phase 2 为 `crab-api` 新增了多模型回退与错误分类层，在原有 `LlmBackend` 枚举之上叠加：
...
**已规划（未实现）**：provider_router、cost_optimizer、middleware、response_cache、A/B compare 等高级路由功能。

### 11.2 MCP 协议栈（crab-mcp）

Phase 2 将 MCP 从基础 client/server 扩展，当前已实现的协议扩展：
...
**已规划（未实现）**：tool_version、tool_sandbox、tool_chain、resource_subscription、transport_failover、connection_pool、audit 等。

### 11.3 Agent 可靠性增强（crab-agent）

Phase 2 当前保留的 agent 增强模块聚焦于可靠性：
...
**已规划（未实现）**：提示词工程子系统（adaptive_prompt、prompt_template、prompt_optimizer、prompt_cache）、代码智能子系统（project_context、smart_context、code_nav、code_gen、code_review）、工具智能（tool_analytics、tool_recommender、tool_patterns）等。

### 11.4 TUI 组件库（crab-tui，21 组件）

Phase 2 当前 TUI 共 21 个组件（10 个基础 + 11 个 Phase 2 增强）：
...
**已规划（未实现）**：context_menu、diff_view、file_tree、file_preview、project_browser、session_list/panel/switcher/info、timeline、markdown_enhanced 等。
...
**已规划（未实现）**：rotation（凭证自动轮换）、key_derive（HKDF 密钥派生）、audit（安全审计日志）。

### 11.6 i18n 框架（已规划）

i18n 框架（`i18n.rs` + `localized_error.rs`）已在清理中移除，计划在后续 Phase 2 迭代中重新实现。

### 11.7 core EventBus（已规划）

`event_bus.rs`（发布/订阅事件总线）和 `event_filter.rs`（事件过滤器）已在清理中移除，计划在后续 Phase 2 迭代中重新实现。当前事件传递通过 `tokio::sync::mpsc` 通道直接完成。
...
Phase 2 (进行中 2026-Q2): CLI 功能深度增强 ──────────────────────────
    └── 多模型回退, MCP 协议扩展, Agent 可靠性增强
    └── TUI 21 组件, Auth 云平台, 配置热重载
    └── 清理: 移除 ~98 过度工程模块，聚焦核心功能
    └── 当前: ~2654 测试, ~176 模块
