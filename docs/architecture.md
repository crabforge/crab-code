...
| **Layer 2** 服务层 | `tools` `mcp` `api` `fs` `process` `plugin` `telemetry` `tui` | 工具系统、MCP 完整协议栈、多模型 API 客户端、文件/进程操作、TUI 33 组件 |
| **Layer 1** 基础层 | `core` `common` `config` `auth` | 领域模型、i18n 框架、配置热重载/Profile、Auth 安全增强 |
...
3. **Workspace 隔离** — 14 个 library crate 职责正交（Phase 2 新增 200+ 模块），增量编译只触发改动部分
...
│   │       ├── path.rs                # 跨平台路径规范化
│   │       ├── id.rs                  # ULID 生成
│   │       ├── i18n.rs                # [Phase 2] 国际化框架，locale 加载
│   │       └── localized_error.rs     # [Phase 2] 带本地化消息的错误类型
│   │
│   ├── core/                          # crab-core: 领域模型
│   │   ├── Cargo.toml
│   │   └── src/
...
│   │       ├── event_bus.rs           # [Phase 2] 发布/订阅事件总线 (tokio broadcast)
│   │       └── event_filter.rs        # [Phase 2] 事件过滤器，按类型/优先级路由
│   │
...
│   │       ├── profile.rs             # [Phase 2] 环境 profile (dev/staging/prod)
│   │       ├── permission_rules.rs    # [Phase 2] 增强权限规则持久化
│   │       ├── permission_inherit.rs  # [Phase 2] 权限继承链
│   │       ├── glob_matcher.rs        # [Phase 2] 高性能 glob 匹配器
│   │       ├── plugin_registry.rs     # [Phase 2] 插件注册表 + 生命周期
│   │       ├── migration.rs           # [Phase 2] 设置版本迁移
│   │       ├── schema_gen.rs          # [Phase 2] JSON Schema 生成
│   │       ├── validation.rs          # [Phase 2] 设置验证框架
│   │       ├── completion.rs          # [Phase 2] 设置补全提示
...
│   │       ├── rotation.rs            # [Phase 2] 凭证自动轮换
│   │       ├── key_derive.rs          # [Phase 2] 密钥派生（HKDF）
│   │       └── audit.rs               # [Phase 2] 安全审计日志
│   │
...
│   │       ├── parallel.rs            # [Phase 2] 并行工具执行协调
│   │       ├── token_budget.rs        # [Phase 2] token 预算管理
│   │       ├── response_cache.rs      # [Phase 2] LRU 响应缓存
│   │       ├── dedup.rs               # [Phase 2] 并发相同请求去重
│   │       ├── batch.rs               # [Phase 2] 批量 API 调用队列
│   │       ├── fallback.rs            # [Phase 2] 多模型回退链
│   │       ├── model_selector.rs      # [Phase 2] 动态模型选择策略
│   │       ├── capabilities.rs        # [Phase 2] 模型能力协商与发现
│   │       ├── provider_health.rs     # [Phase 2] 提供商健康检测
│   │       ├── provider_router.rs     # [Phase 2] 提供商自动切换路由
│   │       ├── cost_optimizer.rs      # [Phase 2] 成本优化器（按成本选模型）
│   │       ├── context_optimizer.rs   # [Phase 2] 上下文窗口优化 + 智能截断
│   │       ├── middleware.rs          # [Phase 2] 请求/响应拦截器管道
│   │       ├── output_format.rs       # [Phase 2] 流式输出格式化 + Markdown 管道
│   │       ├── retry_strategy.rs      # [Phase 2] 增强重试策略
│   │       ├── error_classifier.rs    # [Phase 2] 错误分类（可重试/不可重试）
│   │       ├── degradation.rs         # [Phase 2] 降级模式（API 不可用时）
│   │       ├── stream_aggregator.rs   # [Phase 2] 流式响应聚合器
│   │       ├── ab_compare.rs          # [Phase 2] 多模型 A/B 对比
│   │       ├── response_scorer.rs     # [Phase 2] 响应质量评分
│   │       ├── token_limiter.rs       # [Phase 2] 动态 token 限制
│   │       └── usage_stats.rs         # [Phase 2] 使用量统计仪表板
│   │
│   ├── mcp/                           # crab-mcp: MCP 协议
│   │   ├── Cargo.toml
│   │   └── src/
...
│   │       ├── connection_pool.rs     # [Phase 2] 连接池管理
│   │       ├── transport_layer.rs     # [Phase 2] 传输层抽象重构
│   │       ├── transport_failover.rs  # [Phase 2] 传输自动切换
│   │       ├── transport_monitor.rs   # [Phase 2] 传输健康监控
│   │       ├── tool_version.rs        # [Phase 2] 工具版本化管理
│   │       ├── tool_group.rs          # [Phase 2] 工具分组 + 工具搜索
│   │       ├── tool_sandbox.rs        # [Phase 2] 工具执行沙箱 + 权限边界
│   │       ├── tool_chain.rs          # [Phase 2] 工具链组合编排
│   │       ├── resource_subscription.rs # [Phase 2] 资源订阅
│   │       ├── resource_watcher.rs    # [Phase 2] 资源变更监听
│   │       ├── resource_diff.rs       # [Phase 2] 资源增量更新
│   │       └── audit.rs               # [Phase 2] 审计日志
│   │
│   ├── fs/                            # crab-fs: 文件系统
│   │   ├── Cargo.toml
│   │   └── src/
...
│   │       ├── encoding.rs            # [Phase 2] 文件编码检测 + BOM 处理
│   │       ├── line_ending.rs         # [Phase 2] 换行符检测与规范化
│   │       ├── symlink.rs             # [Phase 2] 符号链接处理 + 安全解析
│   │       ├── filetype.rs            # [Phase 2] 文件类型检测（MIME）
│   │       ├── file_stats.rs          # [Phase 2] 文件统计信息
│   │       ├── incremental_hash.rs    # [Phase 2] 增量文件哈希 + 变更检测
│   │       ├── index.rs               # [Phase 2] 文件索引（快速搜索缓存）
│   │       ├── content_cache.rs       # [Phase 2] 文件内容 LRU 缓存
│   │       ├── project_detect.rs      # [Phase 2] 项目类型检测（Rust/Python/JS…）
│   │       └── template.rs            # [Phase 2] 项目模板系统 + 脚手架生成
│   │
...
│   │       ├── progress.rs            # 进度解析 + 实时报告
│   │       ├── streaming_capture.rs   # [Phase 2] 流式输出捕获
│   │       ├── command_history.rs     # [Phase 2] 命令历史 + 统计
│   │       ├── command_alias.rs       # [Phase 2] 命令别名注册 + 解析
│   │       └── command_suggest.rs     # [Phase 2] 智能命令建议
│   │
...
│   │       ├── analytics.rs           # [Phase 2] 会话分析 + 指标 + 主题分段
│   │       ├── quality.rs             # [Phase 2] 对话质量评估
│   │       ├── auto_summary.rs        # [Phase 2] 规则驱动自动摘要生成
│   │       ├── template.rs            # [Phase 2] 会话模板 + 快速恢复
│   │       └── tag.rs                 # [Phase 2] 会话标签存储
│   │
...
│   │       ├── work_stealing.rs       # 工作窃取调度器
│   │       ├── health.rs              # Agent 健康监控
│   │       ├── system_prompt.rs       # 系统提示构建 + CRAB.md 注入
│   │       ├── assignment.rs          # 任务分配策略
│   │       ├── conversation_tree.rs   # 对话分支树
│   │       ├── summary.rs             # [Phase 2 alias] = summarizer
│   │       ├── summarizer.rs          # [Phase 2] 对话摘要生成
│   │       ├── memory_retriever.rs    # [Phase 2] 长期记忆检索增强
│   │       ├── smart_context.rs       # [Phase 2] 智能上下文注入（相关代码片段）
│   │       ├── project_context.rs     # [Phase 2] 项目理解上下文
│   │       ├── code_nav.rs            # [Phase 2] 代码导航（定义/引用查找）
│   │       ├── code_gen.rs            # [Phase 2] 代码生成模板
│   │       ├── code_review.rs         # [Phase 2] 代码审查辅助
│   │       ├── snippet_store.rs       # [Phase 2] 代码片段库
│   │       ├── prompt_template.rs     # [Phase 2] 提示词模板系统
│   │       ├── prompt_optimizer.rs    # [Phase 2] 提示词优化器
│   │       ├── prompt_cache.rs        # [Phase 2] 提示词缓存
│   │       ├── adaptive_prompt.rs     # [Phase 2] 自适应系统提示
│   │       ├── tool_analytics.rs      # [Phase 2] 工具使用分析
│   │       ├── tool_recommender.rs    # [Phase 2] 自动工具推荐
│   │       ├── tool_patterns.rs       # [Phase 2] 工具使用模式学习
│   │       ├── tool_pipeline.rs       # [Phase 2] 工具执行管道 + 工具链
│   │       ├── branch_manager.rs      # [Phase 2] 会话分支管理器
│   │       ├── checkpoint.rs          # [Phase 2] 检查点系统
│   │       ├── rollback.rs            # [Phase 2] 回滚机制
│   │       ├── dialogue.rs            # [Phase 2] 多轮对话状态机
│   │       ├── error_recovery.rs      # [Phase 2] 错误恢复策略
│   │       ├── retry.rs               # [Phase 2] 自动重试机制
│   │       ├── repl_commands.rs       # [Phase 2] REPL 命令（/undo /branch /fork）
│   │       └── metrics.rs             # [Phase 2] Agent 性能指标
│   │
│   ├── tui/                           # crab-tui: 终端 UI（33 组件）
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── app.rs                 # App 状态机，主循环
...
│   │       │   ├── context_menu.rs    # [Phase 2] 右键上下文菜单
│   │       │   ├── diff_view.rs       # [Phase 2] 分屏对比视图
│   │       │   ├── file_preview.rs    # [Phase 2] 文件预览面板
│   │       │   ├── file_tree.rs       # [Phase 2] 文件树面板
│   │       │   ├── input_history.rs   # [Phase 2] 输入历史（↑↓）
│   │       │   ├── loading.rs         # [Phase 2] 加载动画组件
│   │       │   ├── markdown_enhanced.rs # [Phase 2] 增强 Markdown（表格/任务列表）
│   │       │   ├── notification.rs    # [Phase 2] Toast 通知系统
│   │       │   ├── progress_indicator.rs # [Phase 2] 进度指示器
│   │       │   ├── project_browser.rs # [Phase 2] 项目文件浏览器
│   │       │   ├── search.rs          # [Phase 2] 全局搜索面板
│   │       │   ├── session_info.rs    # [Phase 2] 会话信息面板
│   │       │   ├── session_list.rs    # [Phase 2] 会话列表
│   │       │   ├── session_panel.rs   # [Phase 2] 会话管理面板
│   │       │   ├── session_switcher.rs # [Phase 2] 会话快速切换器
│   │       │   ├── shortcut_hint.rs   # [Phase 2] 快捷键提示栏
│   │       │   ├── status_bar.rs      # [Phase 2] 增强状态栏
│   │       │   ├── timeline.rs        # [Phase 2] 会话时间线
...
| 辅助 crate | 1 | `xtask` |
| **合计** | **17** | — |
| Phase 2 新增模块 | 200+ | 分布于 14 个 library crate |
| 测试总数 | 5000+ | `cargo test --workspace`（2026-04-06） |
...
├── path.rs               // 跨平台路径规范化
├── id.rs                 // ULID 生成
├── i18n.rs               // [Phase 2] 国际化框架：locale 检测、翻译表加载
└── localized_error.rs    // [Phase 2] 带本地化消息的错误包装类型
```
...
├── capability.rs     // Agent 能力声明
├── event_bus.rs      // [Phase 2] 发布/订阅事件总线（tokio broadcast channel）
└── event_filter.rs   // [Phase 2] 事件过滤器：按类型、优先级、来源过滤和路由
```
...
├── profile.rs            // [Phase 2] 环境 profile 系统（dev/staging/prod）
├── permission_rules.rs   // [Phase 2] 增强权限规则持久化
├── permission_inherit.rs // [Phase 2] 权限继承链（父目录→子目录）
├── glob_matcher.rs       // [Phase 2] 高性能 glob 匹配器（globset 封装）
├── plugin_registry.rs    // [Phase 2] 插件注册表 + 生命周期管理
├── migration.rs          // [Phase 2] 设置版本迁移框架
├── schema_gen.rs         // [Phase 2] JSON Schema 生成（schemars）
├── validation.rs         // [Phase 2] 设置验证框架
├── completion.rs         // [Phase 2] 设置 key 补全提示
└── permissions.rs        // [Phase 2] 权限决策统一入口
```
...
├── rotation.rs           // [Phase 2] 凭证自动轮换（后台定时检查）
├── key_derive.rs         // [Phase 2] 密钥派生（HKDF-SHA256）
└── audit.rs              // [Phase 2] 安全审计日志（认证操作记录）
```
...
├── parallel.rs           // [Phase 2] 并行工具执行协调
├── token_budget.rs       // [Phase 2] token 预算管理（请求级别上限控制）
├── response_cache.rs     // [Phase 2] LRU 响应缓存（相同 prompt 命中缓存）
├── dedup.rs              // [Phase 2] 并发相同请求去重（inflight map）
├── batch.rs              // [Phase 2] 批量 API 调用队列
├── fallback.rs           // [Phase 2] 多模型回退链（主模型失败→备用模型）
├── model_selector.rs     // [Phase 2] 动态模型选择策略
├── capabilities.rs       // [Phase 2] 模型能力协商与发现
├── provider_health.rs    // [Phase 2] 提供商健康检测（延迟/错误率）
├── provider_router.rs    // [Phase 2] 提供商自动切换路由
├── cost_optimizer.rs     // [Phase 2] 成本优化器（按成本选最优模型）
├── context_optimizer.rs  // [Phase 2] 上下文窗口优化 + 智能截断策略
├── middleware.rs         // [Phase 2] 请求/响应可插拔拦截器管道
├── output_format.rs      // [Phase 2] 流式输出格式化 + Markdown 渲染管道
├── retry_strategy.rs     // [Phase 2] 增强重试策略（退避 + jitter）
├── error_classifier.rs   // [Phase 2] 错误分类（可重试/不可重试/速率限制）
├── degradation.rs        // [Phase 2] 降级模式（API 不可用时的 fallback 行为）
├── stream_aggregator.rs  // [Phase 2] 多流聚合器
├── ab_compare.rs         // [Phase 2] 多模型 A/B 对比测试
├── response_scorer.rs    // [Phase 2] 响应质量评分（长度/连贯性/相关性）
├── token_limiter.rs      // [Phase 2] 动态 token 限制（模型能力查询）
└── usage_stats.rs        // [Phase 2] 使用量统计仪表板（日/周/月聚合）
```
...
├── connection_pool.rs      // [Phase 2] 连接池管理（多并发请求）
├── transport_layer.rs      // [Phase 2] 传输层抽象重构
├── transport_failover.rs   // [Phase 2] 传输自动切换（主备 transport）
├── transport_monitor.rs    // [Phase 2] 传输健康监控（延迟/错误率）
├── tool_version.rs         // [Phase 2] 工具版本化管理（semver）
├── tool_group.rs           // [Phase 2] 工具分组 + 工具搜索（标签/类别）
├── tool_sandbox.rs         // [Phase 2] 工具执行沙箱 + 权限边界
├── tool_chain.rs           // [Phase 2] 工具链组合编排（sequential/parallel）
├── resource_subscription.rs // [Phase 2] 资源订阅（resources/subscribe）
├── resource_watcher.rs     // [Phase 2] 资源变更监听
├── resource_diff.rs        // [Phase 2] 资源增量更新（patch）
└── audit.rs                // [Phase 2] 审计日志（工具调用记录）
```
...
├── encoding.rs           // [Phase 2] 文件编码检测 + BOM 处理（chardet）
├── line_ending.rs        // [Phase 2] 换行符检测（LF/CRLF/CR）与规范化
├── symlink.rs            // [Phase 2] 符号链接处理 + 安全路径解析（防逃逸）
├── filetype.rs           // [Phase 2] 文件类型检测（MIME、语言分类）
├── file_stats.rs         // [Phase 2] 文件统计信息（大小、行数、修改时间）
├── incremental_hash.rs   // [Phase 2] 增量文件哈希 + 变更检测（xxhash3）
├── index.rs              // [Phase 2] 文件索引（快速搜索缓存，内存 BTreeMap）
├── content_cache.rs      // [Phase 2] 文件内容 LRU 缓存
├── project_detect.rs     // [Phase 2] 项目类型检测（Rust/Python/JS/Go/…）
└── template.rs           // [Phase 2] 项目模板系统 + 脚手架生成
```
...
├── sandbox.rs            // 沙箱策略 (feature = "sandbox")
├── progress.rs           // 进度解析 + 实时报告
├── streaming_capture.rs  // [Phase 2] 流式输出捕获（行缓冲 + 实时回调）
├── command_history.rs    // [Phase 2] 命令历史记录 + 统计（频率/最近使用）
├── command_alias.rs      // [Phase 2] 命令别名注册 + 解析
└── command_suggest.rs    // [Phase 2] 智能命令建议（历史 + 频率 + 前缀匹配）
```
...
├── cost.rs           // token 计数、费用追踪
├── analytics.rs      // [Phase 2] 会话分析 + 指标 + 主题分段
├── quality.rs        // [Phase 2] 对话质量评估（连贯性/相关性评分）
├── auto_summary.rs   // [Phase 2] 规则驱动自动摘要生成
├── template.rs       // [Phase 2] 会话模板 + 快速恢复
└── tag.rs            // [Phase 2] 会话标签存储
```
...
├── work_stealing.rs      // 工作窃取调度器（任务均衡）
├── health.rs             // Agent 健康监控
├── system_prompt.rs      // 系统提示构建 + CRAB.md 注入
├── assignment.rs         // 任务分配策略（轮询/能力匹配）
├── conversation_tree.rs  // 对话分支树（/branch /fork 实现基础）
├── summarizer.rs         // [Phase 2] 对话摘要生成
├── memory_retriever.rs   // [Phase 2] 长期记忆检索增强（语义搜索）
├── smart_context.rs      // [Phase 2] 智能上下文注入（相关代码片段）
├── project_context.rs    // [Phase 2] 项目理解上下文（语言/框架/结构）
├── code_nav.rs           // [Phase 2] 代码导航（定义/引用查找）
├── code_gen.rs           // [Phase 2] 代码生成模板引擎
├── code_review.rs        // [Phase 2] 代码审查辅助（风格/安全/性能）
├── snippet_store.rs      // [Phase 2] 代码片段库（复用 + 检索）
├── prompt_template.rs    // [Phase 2] 提示词模板系统（Handlebars 语法）
├── prompt_optimizer.rs   // [Phase 2] 提示词优化器（动态裁剪）
├── prompt_cache.rs       // [Phase 2] 提示词缓存（相同系统提示去重）
├── adaptive_prompt.rs    // [Phase 2] 自适应系统提示（按上下文动态调整）
├── tool_analytics.rs     // [Phase 2] 工具使用分析（频率/成功率/延迟）
├── tool_recommender.rs   // [Phase 2] 自动工具推荐（基于任务类型）
├── tool_patterns.rs      // [Phase 2] 工具使用模式学习
├── tool_pipeline.rs      // [Phase 2] 工具执行管道 + 工具链（sequential/parallel）
├── branch_manager.rs     // [Phase 2] 会话分支管理器（/branch /fork）
├── checkpoint.rs         // [Phase 2] 检查点系统（保存/恢复会话状态）
...
└── metrics.rs            // [Phase 2] Agent 性能指标（延迟/工具成功率/token）
```
...
│   ├── context_menu.rs     // [Phase 2] 右键上下文菜单
│   ├── diff_view.rs        // [Phase 2] 分屏对比视图（左右两栏内联 diff）
│   ├── file_preview.rs     // [Phase 2] 文件预览面板（只读，语法高亮）
│   ├── file_tree.rs        // [Phase 2] 文件树面板（项目目录树导航）
│   ├── input_history.rs    // [Phase 2] 输入历史（↑↓ 浏览历史输入）
│   ├── loading.rs          // [Phase 2] 加载动画组件（多种动画样式）
│   ├── markdown_enhanced.rs // [Phase 2] 增强 Markdown（表格/任务列表/折叠）
│   ├── notification.rs     // [Phase 2] Toast 通知系统（顶部弹出/自动消失）
│   ├── progress_indicator.rs // [Phase 2] 进度指示器（百分比 + 进度条）
│   ├── project_browser.rs  // [Phase 2] 项目文件浏览器（集成文件树+预览）
│   ├── search.rs           // [Phase 2] 全局搜索面板（内容/文件名搜索）
│   ├── session_info.rs     // [Phase 2] 当前会话信息面板
│   ├── session_list.rs     // [Phase 2] 历史会话列表
│   ├── session_panel.rs    // [Phase 2] 会话管理面板（查看/切换/删除）
│   ├── session_switcher.rs // [Phase 2] 会话快速切换器（弹出式）
│   ├── shortcut_hint.rs    // [Phase 2] 快捷键提示栏（底部常显）
│   ├── status_bar.rs       // [Phase 2] 增强状态栏（模式/provider/token/延迟）
│   ├── timeline.rs         // [Phase 2] 会话时间线（消息时间轴可视化）
...
Phase 2 为 `crab-api` 新增了完整的多模型智能路由层，在原有 `LlmBackend` 枚举之上叠加：
...
└────────┬────────┘
    ...
    └────┬────────────────┘
         │
    ┌────▼────────────────┐
    │  cost_optimizer.rs  │  — 按 cost/quality 选最优模型
    └────┬────────────────┘
         │
    ┌────▼────────────────┐
    │  middleware.rs      │  — 请求/响应拦截器管道（日志/限流/缓存）
    └────┬────────────────┘
         │
    ┌────▼────────────────┐
    │   response_cache    │  — LRU 缓存（相同 prompt 秒级返回）
    └─────────────────────┘
```

**A/B 对比测试**（`ab_compare.rs`）：并行发送同一请求给两个模型，`response_scorer.rs` 评分选优，积累数据用于模型选择策略优化。

**token 管理三件套**：
- `token_budget.rs` — 请求级别 token 上限控制
- `token_limiter.rs` — 从模型能力协商获取动态限制
- `context_optimizer.rs` — 超出上限时智能截断（保留最近 N 轮 + 摘要）

### 11.2 MCP 完整协议栈（crab-mcp）

Phase 2 将 MCP 从基础 client/server 扩展为完整协议栈：
...
| `tool_version.rs` | 工具语义版本化（semver 兼容性检查）|
| `tool_sandbox.rs` | 工具执行沙箱（权限边界 + 审计）|
| `tool_chain.rs` | 工具链（sequential + parallel 组合编排）|
| `resource_subscription.rs` | 资源订阅（`resources/subscribe`）|
| `transport_failover.rs` | 传输自动切换（stdio ↔ SSE ↔ WebSocket）|
| `connection_pool.rs` | 连接池（多并发 tool_call 共享 transport）|
| `cancellation.rs` | 请求取消（`$/cancelRequest` JSON-RPC 通知）|

### 11.3 Agent 智能增强系统（crab-agent）

Phase 2 为 agent 新增 22 个模块，形成三个子系统：

**提示词工程子系统**：
```
adaptive_prompt → prompt_template → prompt_optimizer → prompt_cache
     │                                     │
     └── 根据上下文动态调整 ─────────────── └── 相同提示去重
```
...
**代码智能子系统**：
```
project_context → smart_context → code_nav → code_gen → code_review
      │                 │              │
      └── 项目结构理解  └── 注入相关代码 └── 定义/引用查找
```

**可靠性子系统**：
```
branch_manager → checkpoint → rollback
error_recovery → retry → dialogue（状态机）
```

工具智能：`tool_analytics` 收集工具使用数据，`tool_recommender` 基于任务类型自动推荐工具，`tool_patterns` 学习常见工具组合模式。

### 11.4 TUI 组件库（crab-tui，33 组件）

Phase 2 将 TUI 组件从 10 个扩展至 33 个，形成完整的组件库：
...
**内容展示组件**：
- `diff_view` — 分屏内联 diff（左右两栏，使用 `similar` crate）
- `markdown_enhanced` — 增强 Markdown（GFM 表格、任务列表、折叠块）
- `code_block` — 代码块 + 复制按钮（syntect 高亮）
- `tool_output` — 工具输出折叠显示（可展开/收起）
- `timeline` — 消息时间轴可视化

**文件系统组件**：
- `file_tree` — 项目目录树（展开/折叠，图标区分文件类型）
- `file_preview` — 只读文件预览（语法高亮）
- `project_browser` — 集成 file_tree + file_preview 的复合面板

**会话管理组件**：
- `session_list` + `session_panel` + `session_switcher` + `session_info`
- `timeline` — 当前会话时间轴
...
安全增强:
  rotation.rs → 凭证自动轮换（后台 tokio::task，过期前 10% 时间触发刷新）
  key_derive.rs → HKDF-SHA256 密钥派生（从主密钥派生服务专用密钥）
  audit.rs → 所有认证操作记录（成功/失败/来源 IP/时间戳）
```

### 11.6 i18n 框架（crab-common）

`i18n.rs` 提供轻量国际化框架：
- locale 自动检测（`LANG`/`LC_ALL` 环境变量 → 系统 locale → 默认 `en`）
- 翻译表采用内嵌 TOML（`include_str!`，零运行时文件 I/O）
- `localized_error.rs` 包装 `Error` 类型，在显示时根据 locale 切换错误消息

当前支持语言：zh-CN（简体中文）、en（英文）。

### 11.7 core EventBus（crab-core）

`event_bus.rs` 基于 `tokio::sync::broadcast` 实现发布/订阅总线：

```rust
// 用法示例
let bus = EventBus::new(256); // channel capacity
let rx = bus.subscribe();      // 任意 crate 可订阅
bus.publish(Event::ToolUseStart { id, name }); // agent 发布

// event_filter.rs 提供过滤器
let filtered = rx.filter(EventFilter::only_tool_events());
```

与原有 `event.rs`（领域事件枚举定义）的关系：`event.rs` 定义消息格式，`event_bus.rs` 提供传输机制，`event_filter.rs` 提供路由规则。

---

## 十一（续）、未来扩展路径（原第十一节）

> （原"十一"节内容不变，编号调整为补充章节）

```
Phase 1 (完成 2026-Q1): CLI 终端工具 ────────────────────────────────
    └── crates/cli + crates/* (14 library crates)
    └── 成果: Agent 循环, MCP, TUI, 945+ 测试

Phase 2 (完成 2026-Q2): CLI 功能深度增强 ────────────────────────────
    └── 多模型支持, MCP 完整协议栈, Agent 智能增强
    └── TUI 33 组件, Auth 安全, i18n, 配置热重载
    └── 成果: 5000+ 测试, 200+ 新模块

Phase 3: Tauri 桌面应用 ─────────────────────────────────────────────
    └── crates/desktop/ (Tauri 2.0)
    └── 复用: core, api, session, tools, config, agent, mcp（全部 Phase 2 成果）
...
Phase 2 (完成 2026-04-06):
  [P2-A] 多模型支持 ────────── [完成] fallback chain, A/B compare, provider health, cost optimizer
  [P2-B] API 增强 ─────────── [完成] response cache, dedup, batch, middleware, retry strategy
  [P2-C] Auth 安全 ────────── [完成] Bedrock/Vertex, AWS IAM, GCP Identity, rotation, audit
  [P2-D] MCP 完整协议栈 ────── [完成] sampling, roots, logging, SSE server, tool versioning/sandbox/chain
                                       resource subscription/watcher, transport failover, handshake/negotiation
  [P2-E] Agent 智能增强 ───── [完成] prompt template/optimizer/cache, tool analytics/recommender,
                                       code gen/nav/review, branch/checkpoint/rollback, error recovery
  [P2-F] Session 增强 ─────── [完成] analytics, quality, auto summary, template, tag
  [P2-G] TUI 组件库 ──────────[完成] 33 components, 命令面板, 文件树, 会话管理, diff view
  [P2-H] Config 增强 ─────── [完成] hot reload, profile, permission rules/inherit, schema gen, migration
  [P2-I] FS 增强 ──────────── [完成] encoding, line ending, symlink, incremental hash, index, template
  [P2-J] Process 增强 ─────── [完成] command history/alias/suggest, streaming capture
  [P2-K] Core 增强 ───────────[完成] EventBus, EventFilter
  [P2-L] i18n 框架 ────────── [完成] common::i18n + localized_error
  总测试数: 5000+（较 Phase 1 的 945+ 增长 5x）
```
