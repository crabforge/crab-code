...
//!
//! Operators are commands that act on a motion range. The full
//! operator + motion execution is deferred to a later milestone;
//! this module defines the operator enum for use by the key handler.

/// A vim operator that expects a motion to define its range.
...
impl Operator {
    /// Whether this operator should transition to insert mode after execution.
    #[must_use]
    pub const fn enters_insert(self) -> bool {
        matches!(self, Self::Change)
    }

    /// Single-character key that triggers this operator.
    #[must_use]
...
/// Try to parse a character into an `Operator`.
#[must_use]
